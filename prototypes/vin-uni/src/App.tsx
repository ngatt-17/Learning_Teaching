...
function App() {
  ...
  const [userName, setUserName] = useState('Thanh Tùng');
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        {selectedCourse ? (
          ...
          <main className="flex-1 p-4 sm:p-6 md:p-8 max-w-7xl w-full mx-auto overflow-y-auto">
            {/* Header Row with Greeting & Notifications Bell Icon centered vertically between Hi and Course overview */}
            <div className="flex items-center justify-between pb-1 mb-3">
              ...
              {/* Centered Vertically Notification Icon with proper size */}
              <button
                className="relative p-2.5 text-slate-600 hover:text-[#1E3A6E] hover:bg-slate-200/60 rounded-full transition-colors flex items-center justify-center shrink-0 border border-slate-200/80 bg-white shadow-2xs cursor-pointer"
                title="Notifications"
              >
                <Bell size={20} />
                <span className="absolute -top-0.5 -right-0.5 bg-[#C8232C] text-white text-[10px] font-bold rounded-full w-4.5 h-4.5 flex items-center justify-center border-2 border-white">
                  2
                </span>
              </button>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 mb-6">
              ...
              <div className="flex items-center gap-3 flex-1 max-w-xl">
                ...
                {/* Filter 'All': Chữ đen, Nền trắng thay vì gray như yêu cầu */}
                <div className="relative shrink-0">
                  <select
                    value={filterCategory}
                    onChange={(e) => setFilterCategory(e.target.value)}
                    className="appearance-none bg-white text-slate-900 font-semibold border border-slate-300 hover:border-slate-400 text-sm py-2 pl-3.5 pr-8 rounded-lg cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#1E3A6E]/20 shadow-2xs transition-all"
                  >
