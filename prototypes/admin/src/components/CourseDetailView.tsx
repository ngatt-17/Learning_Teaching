...
export const CourseDetailView: React.FC<CourseDetailViewProps> = ({ course, onBack }) => {
  const [courseTab, setCourseTab] = useState<'Home' | 'Quiz' | 'Students' | 'Feedback'>('Home');
  ...
  return (
    <div className="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-3.5 shrink-0 w-full flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-900 border border-amber-300">
      ...
      <div className="flex-1 flex w-full overflow-hidden">
        ...
        <aside className="w-48 sm:w-56 bg-white border-r border-slate-200 p-4 shrink-0 flex flex-col justify-between h-full">
          <nav className="space-y-1.5">
            ...
            {/* Tab 4: Feedback (Admin Exclusive Feature) */}
            <button
              onClick={() => setCourseTab('Feedback')}
              className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-semibold transition-colors text-left cursor-pointer ${
                courseTab === 'Feedback'
                  ? 'bg-[#1E3A6E] text-white shadow-xs'
                  : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
            ...
            >
              <div className="flex items-center gap-2">
                <MessageSquareHeart size={16} className={courseTab === 'Feedback' ? 'text-amber-300' : 'text-[#C8232C]'} />
                <span>Feedback</span>
              </div>
              <span
        ...
        <main className="flex-1 overflow-y-auto w-full">
          <div className="max-w-4xl mx-auto p-6 sm:p-8 space-y-6">
            ...
            {/* TAB: FEEDBACK (Admin Exclusive: Phản hồi về Thầy cô, Bài giảng, Chất lượng, Cơ sở vật chất) */}
            {courseTab === 'Feedback' && (
              <CourseFeedbackView course={course} />
            )}
          </div>
        </main>
      </div>
